...
let strengths = []; //now a local variable instead of a global, stores the user's strengths for their hobbies here
let hobbies = []; //now a local variable instead of a global, stores the user's hobbies from their choices here
let no_of_hobbies = 10; //the number of times the program will be looped through

//higherHobbies and higherStrengths are no longer needed since they are not useful anymore, have been taken out
...
const quiz = document.getElementById('quiz')//method returns an element with a specified value, used to edit or read a html element, in this case manipulate quiz
...
let currentQuiz = 0//creates local variable assigned to 0, to act as a 'count' of the number of the question in the quiz
...
loadQuiz(questions[currentQuestionPattern], answers[currentQuestionPattern], hobbies, currentQuiz) //whilst this still takes in four paremeters, when taking in the questions and answers for the questions/answers array, it look at the currentQuestionPattern to decide which one it chooses


submitBtn.addEventListener('click', () => { //this  attaches an event handler to the document, 'click[ing]' being the actual event (so clicking the sumbit button) and the function being what is below
  const answer = getSelected() //function from earlier that takes the userinput from what they have selected
  if (answer) { //makes sure that its the answer that gets assigned this
    if (currentQuestionPattern == 0) {
      ...
      strengths.splice(currentQuiz, 1, strengths[currentQuiz] + answerScores[answer]);
    ...
    if (currentQuiz < no_of_hobbies) {
      ...
      loadQuiz(questions[currentQuestionPattern], answers[currentQuestionPattern], hobbies, currentQuiz);
    } else {
      // we got to the last question of the current question pattern
      if (currentQuestionPattern < questions.length - 1) { // as long as we did not get to the last question pattern, filter hobbies and continue with next question pattern
        ...
        no_of_hobbies = hobbies.length                            // the new number of hobbies is equal to the length of the filtered hobbies array
        ...
        // when we reach the last question of the last question pattern, the quiz is completed
...
for (counter = 0; counter < 8; counter ++){
    if (counter === 0){
      quiz(outdoor,outdoorQuestions,outdoorAnswers);
    
  } else if (counter === 1){

      quiz(sports,sportsQuestions,sportsAnswers);
    
  } else if (counter ===2){

      quiz(series,seriesQuestions,seriesAnswers);
      
  } else if (counter ===3){

      quiz(films,filmQuestions,filmAnswers);
    
  } else if (counter === 4){

      quiz(educational,educationalQuestions,educationalAnswers);
      
  } else if (counter === 5){

      quiz(misc,miscQuestions,miscAnswers);

  } else if (counter === 6){

      quiz(gaming,gamingQuestions,gamingAnswers);
 
  } else if (counter === 7){

      quiz(indoor,indoorQuestions,indoorAnswers);  

  } else if (counter === 7){

      quiz(music,musicQuestions,musicAnswers);
